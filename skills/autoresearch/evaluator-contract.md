...
| `score` | number | no (but recommended) | Numeric metric value for comparison |
...
**`score_improvement` (default):** Keep the experiment only if `score` strictly exceeds the previous best score. If `score` is absent from the JSON, fall back to `pass` field only.
...
print(json.dumps({"pass": median < 0.5, "score": median}))
```
...
print(json.dumps({"pass": rmse < 0.05, "score": rmse}))
```
