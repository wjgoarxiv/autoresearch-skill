...
<p align="center">
  <a href="#언제-사용하는가">언제 사용하는가</a> · <a href="#빠른-시작">빠른 시작</a> · <a href="#기능">기능</a> · <a href="#사용법">사용법</a> · <a href="./README.md">English</a>
</p>
<p align="center">
...
대부분의 LLM CLI 도구에는 이미 반복 실행 모드가 내장되어 있습니다 -- `ralph`, `autopilot`, `/loop`, cron 스케줄링 등. 이들은 git, 테스트 러너, 빌드 시스템에 연결된 코드 중심 작업에 적합합니다. autoresearch-skill이 다루는 문제는 다릅니다: 코드 프로젝트 여부와 관계없이, **수치 메트릭과 탐색할 검색 공간이 있는 모든 것**이 대상입니다.

### autoresearch-skill vs 내장 반복 실행 모드

|  | 수동 프롬프팅 | 내장 모드 (ralph, autopilot, team) | autoresearch-skill |
|:---|:---|:---|:---|
| **도메인** | 무엇이든 가능하나, 매 사이클 직접 운전 | 코드 프로젝트 (git + 테스트 + 빌드) | 측정 가능한 메트릭이 있는 모든 도메인 |
...
### autoresearch-skill을 선택할 때

- **수치 메트릭**이 있고, `{"pass": bool, "score": number}`를 출력하는 스크립트가 있을 때 -- 기계적 평가자가 유지/복원 판단에서 LLM 자의적 판단을 제거합니다
- **코드 프로젝트가 아닌** 도메인일 때 -- 시뮬레이션 파라미터 스윕, 문헌 커버리지 갭, 테스트 케이스 기반 프롬프트 튜닝, 데이터 기반 함수 피팅
- Claude Code뿐 아니라 **여러 CLI 플랫폼에서 야간 실행**이 필요할 때
- 진행이 **정체될 것이고**, 에이전트가 멈추지 않고 전략을 전환하길 원할 때
- 모든 반복의 **기계 판독 가능한 감사 기록** (TSV + 추가 전용 로그)이 필요할 때

### 내장 모드를 유지할 때

- 작업이 **버그 수정이나 기능 구현** 중심일 때 -- ralph과 autopilot은 PRD, 수락 기준, 코드 리뷰 워크플로우를 이해합니다
- 여러 에이전트가 **서로 다른 서브태스크를 병렬로** 처리해야 할 때 -- team 모드의 역할입니다
- **측정 가능한 메트릭이 없을 때** -- autoresearch는 반복의 대상이 될 목표가 필요합니다
- **한 번의 시도로 충분할 때** -- 반복 루프가 필요 없는 작업
